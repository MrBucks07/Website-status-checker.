...
handlers._tokens.post = async function (data, callback) {
    ...
    try {
        ...
        if (mobileno && password) {
            ...
            if (userdata.password == hashedPassword) {
                try {
                    ...
                    callback(500, { "Error": "Server error could not create process tokens post req (internal)." });
                }
        ...
        callback(500, { "Error": "Server error could not process token post req(external)." })
    }
...
handlers._tokens.put = async function (data, callback) {
    ...
    if (token && extend) {
        try {
            var tokendata = await tokenModel.findOneAndUpdate({token : token});
            if (tokendata != null) {
                if (tokendata.expires > Date.now()) {
                    //extending token expiration time.
            ...
            callback(500,{"Error" : "Server error could not process token put req."});
        }
    } else {
        callback(400, { "Error": "Missing required fields.(token and extend bool)" });
    }
};
...
handlers._tokens.delete = async function (data, callback) {
    ...
    try {
        ...
        callback(500, { "Error": "Server error could not process token delete req." })
    }
};
...
handlers._checks.post = async function (data, callback) {
    ...
    var timeoutSeconds = typeof (data.Payload.timeoutSeconds) == 'string' && data.Payload.timeoutSeconds % 1 == 0 && data.Payload.timeoutSeconds > 0 && data.Payload.timeoutSeconds <= 5 ? data.Payload.timeoutSeconds : false;
        ...
        try {
            ...
            if (checkdata != null) {
                ...
                if (protocol || url || method || successCodes || timeoutSeconds) {
                    ...
                    handlers.verifyToken(token, checkdata.mobileno, async function (validToken) {
                        if (validToken) {
                            //updating check data.
                            var errOccured = false;
                            //finding user in db and updating.
...
handlers._checks.delete = async function (data, callback) {
    ...
    if (checkId) {
        try {
            //finding check in db.
            var checkdata = await checkModel.findOne({ checkId: checkId });
            var mobileno = checkdata.mobileno;
            if (checkdata != null) {
                //verifying token in header.
                handlers.verifyToken(token,mobileno, async function(validToken){
                    if (validToken) {
                        try {
                            //finding check and deleting.
                            await checkModel.findOneAndDelete({checkId : checkId});
                            await userModel.findOneAndUpdate(
                                {mobileno : mobileno},
                                {
                                    $pull:{
                            ...
                            callback(500,{"Error" : "Server error in deleting check."});
                        }
                    } else {
                        callback(404, { "Error": "Invalid token." });
                    }
