        ...
        if (token) {
            ...
            handlers.verifyToken(token,mobileno,function(validToken){
                if (validToken) {
                    ...
                    //looking up for users.
                    _data.read('users',mobileno,function(err,data){
                        if (err) {
...
handlers._tokens.post = function(data,callback){
    ...
    if (mobileno && password) {
        ...
        //hashing password to check with users password.
        var hashpassword = helpers.hashPassword(password);
        if (hashpassword) {

            //reading user data associated with this mobileno.
            _data.read('users',mobileno,function(err,userdata){
                if (!err && userdata) {
                    
                    //converting userdata to object.
                    var userobj = helpers.parseJsonToObject(userdata);
                    
                    //checking that password provided is true or not.
                    if (userobj.password == hashpassword) {
                        ...
                        callback(404,{"Error" : "Invalid mobileno or password."});
                    }

                } else {
                    callback(404,{"Error" : "User with this mobileno does not exist."});
                }
            });
            
        } else {
            callback(500,{"Error" : "Could not hash password for matching."})
        }
...
handlers.verifyToken = function(token,mobileno,callback){
